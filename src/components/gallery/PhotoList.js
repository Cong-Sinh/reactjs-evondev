...
const PhotoItem = ({ info: { url, isFavorite, id } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <span
        className="absolute z-10 w-8 opacity-0 cursor-pointer right-5 top-5 group-hover:opacity-100 group-hover:visible"
      ...
      <button
        className="absolute invisible p-3 px-6 text-sm font-medium text-black transition-all bg-white rounded-lg opacity-0 bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible"
